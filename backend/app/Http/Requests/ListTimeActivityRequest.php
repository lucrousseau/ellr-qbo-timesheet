...
class ListTimeActivityRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'start_position' => ['sometimes', 'integer', 'min:1'], // @pest-mutate-ignore declarative list query rules
            'max_results' => ['sometimes', 'integer', 'min:1', 'max:'.$maxResults], // @pest-mutate-ignore declarative list query rules
            'refresh' => ['sometimes', 'boolean'], // @pest-mutate-ignore declarative list query rules
        ];
    }
    ...
    {
        return (int) ($this->validated('start_position') ?? 1); // @pest-mutate-ignore validated integer cast
    }
