...
class StoreExpenseRequest extends FormRequest
{
    ...
    {
        return [
            'amount' => ['required', 'numeric', 'gt:0', 'max:999999999.99'], // @pest-mutate-ignore declarative validation rules
            'txn_date' => ['required', 'date'], // @pest-mutate-ignore declarative validation rules
            'payment_type' => ['sometimes', 'string', Rule::in(ExpensePaymentType::values())], // @pest-mutate-ignore declarative validation rules
            'payment_account_ref' => ['required', 'string', 'max:255'], // @pest-mutate-ignore declarative validation rules
            'expense_account_ref' => ['required', 'string', 'max:255'], // @pest-mutate-ignore declarative validation rules
