...
- **Multi-tenant (phase 1)**: each tenant `User` belongs to one `Organization`; platform operators (`is_super_admin`) have `organization_id = null` and manage tenants via `/api/super-admin/organizations` (`OrganizationLifecycleService`). QBO `realm_id` is bound per org via `OrganizationRealmService`. Admin APIs scope by `organization_id` (middleware, route binding on `{user}`, and `OrganizationAccessService`). Realm snapshots purge through `OrganizationRealmDataPurgeService` on disconnect and org delete. See `OrganizationRegistrationService`, `OrganizationRealmService`, `QuickBooksTokenResolverService`.
